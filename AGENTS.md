...
### Setup
```bash
uv sync --dev                # Install all dependencies (Python >=3.12, managed by uv)
```
...
uv run pytest tests/unit/test_pep.py -k "test_name" -q

# Behavioral tests (MUST pass — hard gate for any change)
...
# Start daemon via dev harness (background, requires tmux)
bash runner/harness.sh start
bash runner/harness.sh status
bash runner/harness.sh stop

# Verify provider/policy setup
uv run shisad doctor check --component all
```

### Ruff/Mypy Config
- Python 3.12 target, line length 100
- Ruff selects: E, F, W, I, UP, B, SIM, RUF
- Mypy strict mode with pydantic plugin

## Architecture Overview

shisad is a long-running daemon that sits between an LLM (the "planner") and external systems. The LLM proposes tool calls; the daemon runtime decides what actually executes. The fundamental security boundary is that the LLM is untrusted — prompt injection is assumed to succeed, so enforcement runs in a separate trust domain that never sees raw content.

### Runtime Flow (message → tool execution)

1. **Client** sends message via JSON-RPC over Unix socket (`session.message`)
2. **Ingress firewall** (`security/firewall/`) classifies and taint-labels the input
3. **Context construction** separates trusted (system prompt, user goal) from untrusted (tool output, web content) via spotlighting; large untrusted content stored as opaque evidence references
4. **Planner** (`core/planner.py`) sends context to LLM provider, extracts `<tool_call>` proposals
5. **PEP pipeline** (`security/pep.py`) — 8 layers run on every proposed tool call (first rejection wins):
   - Registry → Schema → Capability → DLP → Resource Authorization → Egress Allowlist → Credential Scoping → Taint Sink
6. **Approval system** (`core/approval.py`) — if PEP returns REQUIRE_CONFIRMATION, routes to user via web UI (passkey/TOTP/local helper)
7. **Executor** (`executors/`) runs the tool in sandbox (browser, shell, HTTP proxy), captures output as tainted UNTRUSTED
8. **Egress firewall** (`security/firewall/output.py`) — DLP scan, taint sink enforcement on output
9. **Control plane** (`security/control_plane/`) — 5 async consensus voters detect behavioral anomalies across the session

### Key Subsystems

| Subsystem | Location | Role |
|-----------|----------|------|
| **Daemon runner** | `daemon/runner.py` | Lifecycle, event loop, service bootstrap |
| **Services factory** | `daemon/services.py` | Wires all subsystems (PEP, firewall, providers, channels, skills, memory) |
| **Handlers** | `daemon/handlers/` | ~200 JSON-RPC methods: `_impl_session.py` (planning loop), `_impl_tool_execution.py`, `_impl_memory.py`, `_impl_confirmation.py`, `_impl_admin.py`, `_impl_skills.py`, etc. |
| **Session model** | `core/session.py` | ORCHESTRATOR (user-facing) vs SUBAGENT (task-isolated) roles; DEFAULT / ADMIN_CLEANROOM / TASK modes |
| **PEP** | `security/pep.py` | Per-call 8-layer enforcement; content-blind (metadata only) |
| **Content firewalls** | `security/firewall/` | Ingress (PromptGuard 2 classifier, taint labeling) + egress (DLP, redaction) |
| **Control plane** | `security/control_plane/` | 5-voter consensus sidecar (trace, sequence, resource, intent, network) |
| **Taint tracking** | `core/types.py` | `TaintLabel` enum (UNTRUSTED, USER_REVIEWED, SENSITIVE_FILE, etc.) flows through all content |
| **Provider routing** | `core/providers/routing.py` | Per-component LLM routing (PLANNER, EMBEDDINGS, MONITOR) with pluggable presets |
| **Tool registry** | `core/tools/` | `ToolDefinition` with schema hash pinning; builtin tools (fs, git, web, browser, shell, memory) |
| **Skills** | `skills/` | Hot-reloadable plugins with vetting pipeline (static analysis, YARA, sandbox, disclosure) |
| **Channels** | `channels/` | Discord, Telegram, Slack, Matrix — default-deny identity allowlisting |
| **Memory** | `memory/` | Semantic search + RAG with taint-aware ingestion (SQLite + embeddings) |
| **Scheduler** | `scheduler/` | TASK delivery with immutable task envelopes, taint-isolated from COMMAND |
| **Executors** | `executors/` | Browser (Playwright), shell, HTTP proxy — sandboxed with iptables/mount/quota |
| **CLI** | `cli/main.py` | Click-based entry point (`shisad` / `shisactl` commands) |
| **Approval web** | `daemon/approval_web.py` + `approver/` | WebAuthn/TOTP approval UI (separate `shisad-approver` process) |

### COMMAND/TASK Isolation

Sessions are either ORCHESTRATOR (holds user intent, dispatches work) or SUBAGENT (ephemeral task execution). Task handoff uses immutable **task envelopes** (`scheduler/schema.py`) — the TASK never sees COMMAND history, only a taint-filtered goal summary. Task results pass through the egress firewall before returning to COMMAND.

### Content-Blind Enforcement (Why It Works)

The PEP and control plane see only metadata (tool names, resource identifiers, timing patterns) — never file contents, message bodies, or free-form text. This means prompt injection cannot influence approval decisions because injected content never reaches the components that make them. The planner is constrained to *proposing* actions, not approving them.
