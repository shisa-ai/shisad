...
# These tools need state that a generic direct-RPC probe cannot safely invent.
# Their product contracts are covered by stateful behavioral journeys; keep the
# omission explicit so a newly registered, genuinely unhandled tool still fails
# the matrix as missing_payload_template.
_FIXTURE_BOUND_TOOL_NAMES: frozenset[str] = frozenset(
    {
        "action.resolve",
        "lockdown.resume",
        "thread.list",
...
def _probeable_tool_names(registered_tools: list[str]) -> list[str]:
    return [name for name in registered_tools if name not in _FIXTURE_BOUND_TOOL_NAMES]
...
def _print_tool_status_markdown(
    rows: list[MatrixRow],
    *,
    ...
    if omitted_tools:
        print()
        print(
            "*Fixture-bound registered tools omitted from this direct probe: "
            f"{', '.join(omitted_tools)}.*"
        )
    print()
...
async def _run_matrix(*, strict_disabled: bool, tool_status: bool = False) -> int:
    ...
    try:
        ...
        registered_tools = [str(item) for item in status.get("tools_registered", [])]
        tools = _probeable_tool_names(registered_tools)
        omitted_tools = [name for name in registered_tools if name not in tools]
    ...
    if tool_status:
        _print_tool_status_markdown(rows, omitted_tools=omitted_tools)
    else:
        print(
            json.dumps(
                {
    ...
    print(
        ...
        f"fail={failed} total={len(rows)} omitted={len(omitted_tools)}"
    )
