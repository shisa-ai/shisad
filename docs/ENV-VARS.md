...
- IP-literal, `localhost`, and `.local` / `.internal` / `.lan` search backend hosts must also be present in `SHISAD_WEB_ALLOWED_DOMAINS`. For local SearxNG, use `SHISAD_WEB_ALLOWED_DOMAINS=127.0.0.1,localhost` in `runner/.env`. Public backend hosts do not need this allowlist entry just to run `web.search`, but listing the backend and common result hosts preapproves backend redirects and later `web.fetch` calls.
