...
- The `ledger` backend uses the Ethereum app's `signTypedData` (EIP-712) to render structured action summaries with labeled fields on the device's trusted display and sign with ECDSA secp256k1. EIP-712 enables Clear Signing for improved display on Stax/Flex devices. Keys registered with `--backend ledger` default to `signing_scheme=eip712` and `algorithm=ecdsa-secp256k1`.
