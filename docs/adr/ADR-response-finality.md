# ADR: Final Answers for Remote No-Action Turns
...
preliminary draft. Before showing a response to the user, it makes one
...
function call is valid. `respond_to_user` is a response protocol, not a runtime
tool: it is never registered, sent through PEP, confirmed, or executed.

Invalid responses receive only the existing limited planner repair attempt.
If no valid final answer is produced, shisad returns its safe planner-validation
...
existing behavior until a separate decision selects a compatible structured
response protocol.
...
closed machine-generated tool-call structure; it does not decide which words
...
- Remote native-tool-capable no-action turns gain an explicit final-answer
  boundary.
- Provider prose outside the validated function call cannot pass through that
  boundary.
