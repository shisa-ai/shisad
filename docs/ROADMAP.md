...
- `v0.8.2` is published. It adds guided setup and startup, session-specific
  chat and Discord progress, config and SQLite upgrades, data backup and
  recovery, audit verification, delivery inspection, and channel
  administration. Current channel adapters still cannot verify uncertain
  deliveries with the provider, and remote web/device enrollment remains
  future work.
...
- v0.8.2 (shipped) — guided setup and a read-only first-run check; explicit
  ...
  model-context failures; clearer final answers and error messages; and
  session-specific confirmation and progress displays in local chat and
  Discord. Setup previews its changes and writes only after explicit approval;
  managed and non-interactive setup never prompts or starts the daemon. Channel
  setup does not claim inbound round-trip verification. Discord threads remain
  off by default, and progress displays omit raw arguments, results,
  destinations, and internal error text. See `CHANGELOG.md` for the complete
  release record.
...
- Onboarding — v0.8.2 includes a read-only first-run check, stored credential
  names, provider/model checks, generated policy profiles, four-channel setup,
  guided interactive setup, deterministic non-interactive setup, background
  startup, a guided tour, and session-specific confirmation and progress views.
  Setup previews changes before writing; managed and non-interactive setup do
  not prompt. `init` remains a minimal template publisher
