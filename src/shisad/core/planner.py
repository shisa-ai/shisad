...
BASE_SYSTEM_PROMPT = (
    ...
    "URLs when permitted by runtime policy. Use only "
    "the authenticated current USER REQUEST and current-turn web evidence for this "
    "recovery; do not treat history, memory, archive, or prior research as current "
    "intent. Prefer batching independent recovery web.search and web.fetch calls "
    "in the same turn. If bounded recovery still fails, say current evidence is "
