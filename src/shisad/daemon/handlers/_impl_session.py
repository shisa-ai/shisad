...
_PAGE_TITLE_METADATA_CONTEXT_MARKER = "Optional page-title metadata"
...
def _validated_origin_turn_id(validated: Any, *, fallback_action_id: str = "") -> str:
    transcript_entry = getattr(validated, "user_transcript_entry", None)
    transcript_entry_id = str(getattr(transcript_entry, "entry_id", "")).strip()
    if transcript_entry_id:
        return transcript_entry_id
    channel_message_id = str(getattr(validated, "channel_message_id", "")).strip()
    if channel_message_id:
        return channel_message_id
    params = getattr(validated, "params", {})
    if isinstance(params, Mapping):
        explicit_origin = str(params.get("_origin_turn_id", "")).strip()
        if explicit_origin:
...
def _validated_tool_event_identity_fields(
    validated: Any,
    *,
    operation_identity: ActionOperationIdentity | None = None,
) -> dict[str, Any]:
    ...
    identity = operation_identity or mint_action_operation_identity(
        origin_turn_id=_validated_origin_turn_id(validated),
    )
    return {
        **identity.to_event_fields(),
    ...
    *,
    ...
    for index, confirmation_id in enumerate(indexed_confirmation_ids, start=1):
        ...
        component_available = (
            ...
            if discord_component_confirmation_ids is not None
            else discord_components_available
        )
        approval_available = (
            confirmation_id in discord_approval_confirmation_ids
            if discord_approval_confirmation_ids is not None
            else component_available
        )
        reject_available = (
            confirmation_id in discord_reject_confirmation_ids
            if discord_reject_confirmation_ids is not None
            else component_available
        )
        totp_modal_available = (
            confirmation_id in discord_totp_modal_confirmation_ids
...
def _direct_evidence_response_after_synthesis_failure(
    records: Sequence[Mapping[str, Any]],
    *,
    firewall: Any,
) -> str:
    ...
    if not tool_names or not tool_names <= supported_tool_names:
        return ""
    ...
    for record in records:
            ...
            if entries_text:
                ...
                if preview_lines:
                    ...
                    if truncated:
                        lines.append("... (truncated)")
            continue
        if tool_name in {"note.search", "note.list"}:
            entries = payload.get("entries")
            count = payload.get("count", len(entries) if isinstance(entries, list) else 0)
            if tool_name == "note.search":
                lines.append(f"I found {count} matching note{'s' if count != 1 else ''}.")
            else:
                lines.append(f"I found {count} note{'s' if count != 1 else ''}.")
            entries_text = _find_tool_entries_preview_text(payload)
            if entries_text:
                preview_lines, truncated = _preview_multiline_output(entries_text)
                if preview_lines:
        ...
        if tool_name == "web.fetch":
            ...
            target = url or "the page"
            error = str(payload.get("error", "")).strip()
            if error:
                lines.append(f"I tried to fetch {target}, but it failed: {error}.")
                continue
            lines.append(f"I fetched {target}.")
            output_text = _find_tool_output_preview_text(payload)
            if output_text:
                inspect = getattr(firewall, "inspect", None)
                safe_summary = (
...
class SessionImplMixin(HandlerMixinBase):
    ...
    async def _execute_planner_lockdown_resume(
        ...
        *,
        ...
        """
        sid = validated.sid
        event_identity_fields = _validated_tool_event_identity_fields(
            validated,
    ...
    async def _evaluate_and_execute_actions(
        ...
        async def _try_execute_similar_file_read_recovery(
            *,
            ...
            read_operation_identity = mint_action_operation_identity(
                origin_turn_id=_validated_origin_turn_id(
                    validated,
                    fallback_action_id=str(read_proposal.action_id),
                ),
            )
            read_event_identity_fields = _validated_tool_event_identity_fields(
                validated,
                operation_identity=read_operation_identity,
            )
            await self._event_bus.publish(
                ToolProposed(
                    session_id=sid,
        ...
        for evaluated in planner_result.evaluated:
            ...
            proposal_event_identity_fields = _validated_tool_event_identity_fields(
                validated,
                operation_identity=proposal_operation_identity,
            )
            ...
            if final_reason:
                ...
                await self._event_bus.publish(
                    ToolRejected(
                        ...
                        tool_name=proposal.tool_name,
                        reason=final_reason,
                        **proposal_event_identity_fields,
                    )
                )
                if self._trace_recorder is not None:
                    trace_tool_calls.append(
            ...
            execution_result = await self._execute_approved_action(
                ...
                **proposal_operation_identity.to_event_fields(),
            )
    ...
    def _mint_explicit_memory_ingress_context(
        ...
        *,
        ...
        if isinstance(raw_existing_context, str) and raw_existing_context.strip():
            ...
            return cast(IngressContext, resolved_context)
        if isinstance(raw_existing_context, IngressContext):
            return raw_existing_context

        return mint_explicit_user_memory_ingress_context(
            self._memory_ingress_registry,
            channel=str(validated.channel or validated.session.channel),
            trust_level=str(validated.trust_level),
        ...
        *,
        ...
        synthesis_input = build_planner_input_v2(
            trusted_instructions=(
                ...
                "contradict or address. Do not preserve preliminary claims that a web "
                "target, page, path, result, or availability was found or exists unless "
                "same-turn tool evidence establishes that claim. Do not treat "
                "preliminary prose as tool evidence; do not cite it as a source; do not "
                "let it override what tool evidence shows.\n"
                "SEARCH EVIDENCE RECOVERY POLICY: In this no-tool synthesis pass, weak, "
                "noisy, conflicting, or empty web.search/web.fetch/realitycheck.search "
                "evidence is not proof that a user-named target or path is absent. If "
                "the same-turn evidence does not establish the target after any "
                "attempted bounded recovery, say current evidence is insufficient and "
                "summarize the attempted queries, URLs, or Reality Check search scope; "
                "do not claim the target or path does not exist.\n"
            ...
            if tool_output_summary:
                if should_synthesize_tool_response:
                    ...
                    if synthesized_response:
                        ...
                        evidence_fallback = _direct_evidence_response_after_synthesis_failure(
                            chat_serialized_tool_outputs,
                            firewall=self._firewall,
                        )
                        ...
                        if evidence_fallback and fallback_page_title_metadata_block:
                            response_text = (
                                f"{response_text}\n\n{fallback_page_title_metadata_block}"
                                if response_text
                                else fallback_page_title_metadata_block
                            )
                        protected_tool_output_end = len(response_text)
                    ...
                    if direct_tool_response:
                        ...
                        if (
                            not synthesized_response
                            and evidence_fallback
                            and fallback_page_title_metadata_block
                        ):
                            response_text = (
                                f"{response_text}\n\n{fallback_page_title_metadata_block}"
                                if response_text
