...
def test_m2_t23_ci_generates_coverage_and_validates_baseline() -> None:
    ...
    assert "strategy" not in lint_and_test
    assert lint_and_test.get("env", {}).get("UV_PYTHON") == "3.12"
    sync_steps = [cmd for cmd in lint_runs if cmd.startswith("uv sync ")]
    assert sync_steps, "lint-and-test must install the Python 3.12 environment"
    assert all("--python 3.12" in cmd for cmd in sync_steps)
