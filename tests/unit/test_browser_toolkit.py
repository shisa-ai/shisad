...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_uses_form_associated_default_submitter(
    tmp_path: Path,
) -> None:
    state = {
        "prefix_html": (
            "<form id='external-form' action='/external' method='get'>"
            "<input id='external-search' name='q' type='text' />"
            "</form>"
            "<button id='external-submit' type='submit' form='external-form' "
            "formaction='/override' formmethod='get'>Go</button>"
        )
    }
    browser_server = _start_fixture_server(state=state)
    try:
        ...
        prepared = await toolkit.prepare_action_arguments(
            session=session,
            tool_name="browser.type_text",
            arguments={"target": "#external-search", "text": "hello", "submit": True},
        )
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_predicts_aria_disabled_default_submitter(
    tmp_path: Path,
) -> None:
    state = {
        "submit_html": (
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_counts_disabled_text_field_blocker(
    tmp_path: Path,
) -> None:
    state = {"submit_html": "<input id='other' name='other' type='text' disabled />"}
    browser_server = _start_fixture_server(state=state)
    try:
        runner = _DirectRunner()
        toolkit = _toolkit(tmp_path, runner=runner)
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_does_not_predict_fieldset_disabled_submitter(
    tmp_path: Path,
) -> None:
    state = {
        "submit_html": (
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_counts_fieldset_disabled_text_field_blocker(
    tmp_path: Path,
) -> None:
    state = {
        "submit_html": (
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_allows_disabled_fieldset_first_legend_submitter(
    tmp_path: Path,
) -> None:
    state = {
        "submit_html": (
            "<fieldset disabled>"
            "<legend><button id='legend-submit' type='submit'>Go</button></legend>"
            "</fieldset>"
        )
    }
    browser_server = _start_fixture_server(state=state)
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_blocks_fieldset_descendant_after_first_legend(
    tmp_path: Path,
) -> None:
    state = {
        "submit_html": (
            ...
            "<div><button id='fieldset-late-submit' type='submit'>Go</button></div>"
            "</fieldset>"
        )
    }
    browser_server = _start_fixture_server(state=state)
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_blocks_void_descendant_after_first_legend(
    tmp_path: Path,
) -> None:
    state = {
        "submit_html": (
            "<fieldset disabled>"
            "<legend><input id='legend-note' name='note' type='text'></legend>"
            "<div><button id='fieldset-late-submit' type='submit'>Go</button></div>"
            "</fieldset>"
        )
    }
...
@pytest.mark.asyncio
async def test_gh24_browser_type_submit_blocks_recovered_descendant_after_first_legend(
    tmp_path: Path,
) -> None:
    state = {
        "submit_html": (
            "<fieldset disabled>"
            "<legend><span>Legend note</legend>"
            "<div><button id='fieldset-late-submit' type='submit'>Go</button></div>"
            "</fieldset>"
        )
    }
...
@pytest.mark.asyncio
async def test_gh24_browser_snapshot_refs_compact_after_disabled_controls(
    tmp_path: Path,
) -> None:
    state = {"prefix_html": "<input id='disabled-first' type='text' disabled />"}
    browser_server = _start_fixture_server(state=state)
    try:
        runner = _DirectRunner()
        toolkit = _toolkit(tmp_path, runner=runner)
...
@pytest.mark.asyncio
async def test_gh24_browser_snapshot_refs_compact_after_hidden_controls(
    tmp_path: Path,
) -> None:
    state = {
        "prefix_html": (
