...
def test_memory_list_requires_owner_scope_before_rpc(
    extra_args: list[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    result = runner.invoke(cli_main.cli, ["memory", "list", *extra_args])
...
def test_memory_list_uses_owner_scope_env_before_rpc(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = _config(tmp_path)
    monkeypatch.setattr(cli_main, "_get_config", lambda: config)
    monkeypatch.setenv("SHISAD_USER", "alice")
    monkeypatch.setenv("SHISAD_WORKSPACE", "ws-1")
    calls: list[tuple[str, dict[str, object] | None]] = []
...
def test_memory_list_flags_override_owner_scope_env(
    ...
    config = _config(tmp_path)
    monkeypatch.setattr(cli_main, "_get_config", lambda: config)
    monkeypatch.setenv("SHISAD_USER", "env-user")
    monkeypatch.setenv("SHISAD_WORKSPACE", "env-ws")
    calls: list[tuple[str, dict[str, object] | None]] = []
...
def test_m9_lockdown_status_rejects_all_with_session(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = _config(tmp_path)
    ...
    result = runner.invoke(cli_main.cli, ["lockdown", "status", "--all", "--session", "s-1"])
...
def test_action_reject_auto_binds_single_allowed_channel_principal(
    ...
    def _fake_rpc_call(
        ...
        *,
        ...
        if method == "action.pending":
            ...
            payload = {"rejected": True, "confirmation_id": "c-1", "status": "rejected"}
        if response_model is None:
            return payload
        return response_model.model_validate(payload)  # type: ignore[attr-defined]
    ...
    result = runner.invoke(cli_main.cli, ["action", "reject", "c-1"])
...
def test_f1_action_reject_retries_unfiltered_nonce_lookup_after_expiry(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = _config(tmp_path)
    ...
    result = CliRunner().invoke(cli_main.cli, ["action", "reject", "c-expired"])
